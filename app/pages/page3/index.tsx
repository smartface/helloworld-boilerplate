...
// HeaderBar View
const HeaderView = (props: NativeStackHeaderProps & { onClosePressed: () => void }) => {
    ...
    return (<View key={"headerView"} style={viewStyle} /* style={[styles['.view'], styles['#page3-headerView']]} */>
        <Pressable
            ...
            style={getStyle(styles, ['.pressable', '#page3-pressable2'], [])}
        >
            <Xmark
                key="icon1"
                height={24}
                width={24}
                style={getStyle(styles, ['.icon', '#page3-icon1'], [])}
            />
        </Pressable>
        <Image
            key="image1"
            source={require('@app/assets/images/smartface.png')}
            style={getStyle(styles, ['.image', '#page3-image1'], [])}
        />
    </View>
    );
};
